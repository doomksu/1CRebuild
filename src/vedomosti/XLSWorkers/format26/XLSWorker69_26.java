...
/**
 * Класс для записи 69х форм в xls файлы  – тип «НЕПОЛУЧ» 
 *
 * @author kneretin
 */
public class XLSWorker69_26 extends XLSWorker {

    private final int FIRST_ROW_NUM = 15;

    public XLSWorker69_26(Workbook _wb, OrganizationsGroup _banks, HashMap<String, String> _params) {
    ...
    protected void writeOrganization(Organization org, Sheet cloneFromSheet, String newSheetName) {
        ...
        //header
        getCellFromReference("D4").setCellValue("Ведомость №___" + orgParams.get("PrilNumber") + "____");
        getCellFromReference("D6").setCellValue("за " + month + " " + year + " г.");
        getCellFromReference("M7").setCellValue(day + "." + monthNum + "." + year);
        getCellFromReference("M8").setCellValue(params.get("KTOPFR"));
        getCellFromReference("M9").setCellValue(params.get("KSP"));
        getCellFromReference("M11").setCellValue(params.get("OKEI"));

        getCellFromReference("D8").setCellValue(params.get("RegNumNameOrg"));
        getCellFromReference("D9").setCellValue(params.get("NameStPodr"));
