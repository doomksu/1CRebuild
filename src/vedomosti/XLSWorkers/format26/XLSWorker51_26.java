...
/**
 * Класс для записи 51х форм в xls файлы - тип «ПЕРЕППОГАШ»
 *
 * @author kneretin
 */
public class XLSWorker51_26 extends XLSWorker {

    private final int FIRST_ROW_NUM = 17;

    public XLSWorker51_26(Workbook _wb, OrganizationsGroup _banks, HashMap<String, String> _params) {
    ...
    protected void writeOrganization(Organization org, Sheet cloneFromSheet, String newSheetName) {
        ...
        getCellFromReference("R5").setCellValue(day + "." + monthNum + "." + year);
        getCellFromReference("R6").setCellValue(params.get("KTOPFR"));
        getCellFromReference("R7").setCellValue(params.get("KSP"));
        getCellFromReference("R10").setCellValue(params.get("OKEI"));
