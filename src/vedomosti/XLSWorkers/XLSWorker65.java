...
/**
 * Класс для записи 65х форм в xls файлы
 *
 * @author kneretin
 */
public class XLSWorker65 extends XLSWorker {

    private final int FIRST_ROW_NUM = 17;

    public XLSWorker65(Workbook _wb, OrganizationsGroup _banks, HashMap<String, String> _params) {
    ...
    protected void writeOrganization(Organization org, Sheet cloneFromSheet, String newSheetName) {
        ...
        //header
        getCellFromReference("A6").setCellValue("за " + month + " " + year + " г.");
        getCellFromReference("G7").setCellValue(day + "." + monthNum + "." + year);
        ...
        getCellFromReference("G9").setCellValue(params.get("KSP"));
        getCellFromReference("G12").setCellValue(params.get("OKEI"));

        getCellFromReference("C8").setCellValue(params.get("RegNumNameOrg"));
        getCellFromReference("C9").setCellValue(params.get("NameStPodr"));
    ...
    private void writeFormToRow(Form form, int strIndex) {
        ...
        for (BigDecimal summ : form.getSumms()) {   //E-G
