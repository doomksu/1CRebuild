...
/**
 * Класс для записи 46х форм в xls файлы
 *
 * @author kneretin
 */
public class XLSWorker46 extends XLSWorker {

    private final int FIRST_ROW_NUM = 15;

    public XLSWorker46(Workbook _wb, OrganizationsGroup _banks, HashMap<String, String> _params) {
    ...
    protected void writeOrganization(Organization org, Sheet cloneFromSheet, String newSheetName) {
        ...
        getCellFromReference("H7").setCellValue(day + "." + monthNum + "." + year);
        getCellFromReference("A4").setCellValue("Ведомость №___" + orgParams.get("PrilNumber") + "____");
        getCellFromReference("H8").setCellValue(params.get("KTOPFR"));
        getCellFromReference("H9").setCellValue(params.get("KSP"));
        getCellFromReference("H11").setCellValue(params.get("OKEI"));

        if (params.get("RegNumNameOrg").length() > 14) {
            getCellFromReference("B9").setCellValue(params.get("RegNumNameOrg").substring(0, 15) + " " + params.get("RegNumNameOrg").substring(15) + " " + params.get("NameStPodr"));
        } else {
            getCellFromReference("B9").setCellValue(params.get("RegNumNameOrg") + " " + params.get("NameStPodr"));
        }
        getCellFromReference("B10").setCellValue(params.get(""));
