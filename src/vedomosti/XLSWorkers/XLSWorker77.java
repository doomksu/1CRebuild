...
/**
 * Класс для записи 77х форм в xls файлы
 *
 * @author kneretin
 */
public class XLSWorker77 extends XLSWorker {

    private final int FIRST_ROW_NUM = 13;

    public XLSWorker77(Workbook _wb, OrganizationsGroup _banks, HashMap<String, String> _params) {
    ...
    protected void writeOrganization(Organization org, Sheet cloneFromSheet, String newSheetName) {
        ...
        getCellFromReference("A2").setCellValue("Ведомость №___" + orgParams.get("PrilNumber") + "____");
        getCellFromReference("H6").setCellValue(params.get("KTOPFR"));
        getCellFromReference("H7").setCellValue(params.get("KSP"));
        getCellFromReference("H9").setCellValue(params.get("OKEI"));

        getCellFromReference("C7").setCellValue(params.get("RegNumNameOrg"));
        getCellFromReference("C8").setCellValue(params.get("NameStPodr"));
